...
GLboolean
RealSquareMatrix::ResizeRows(GLuint row_count)
{
...
GLboolean
RealSquareMatrix::ResizeColumns(GLuint row_count)
{
  return (Matrix<GLdouble>::ResizeRows(row_count) && Matrix<GLdouble>::ResizeColumns(row_count));
}

GLboolean
RealSquareMatrix::PerformLUDecomposition()
{
  ...
  for (vector<vector<GLdouble>>::const_iterator itr = _data.begin();
       ...
       ++itr) {
    GLdouble big = 0.0;
    for (vector<GLdouble>::const_iterator itc = itr->begin(); itc < itr->end();
         ++itc) {
