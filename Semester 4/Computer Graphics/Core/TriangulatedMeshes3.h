...
public:
  ...
  TriangulatedMesh3(GLuint vertex_count = 0,
                    GLuint face_count = 0,
  ...
  // loads the geometry (i.e. the array of vertices and faces) stored in an OFF
  // file at the same time calculates the unit normal vectors associated with
  // vertices
  ...
  GLfloat* MapTextureBuffer(
    GLenum access_flag = GL_READ_ONLY) const; // homework
