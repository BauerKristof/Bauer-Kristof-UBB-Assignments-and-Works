...
TriangulatedMesh3::TriangulatedMesh3(GLuint vertex_count,
                                     GLuint face_count,
                                     GLenum usage_flag)
...
GLvoid
TriangulatedMesh3::DeleteVertexBufferObjects()
{
  if (_vbo_vertices) {
...
GLboolean
TriangulatedMesh3::Render(GLenum render_mode) const
{
  ...
  // render primitives
  glDrawElements(
    render_mode, 3 * (GLsizei)_face.size(), GL_UNSIGNED_INT, (const GLvoid*)0);
                               ...
                               GLboolean translate_and_scale_to_unit_cube)
{
  ...
  // loading faces
  for (vector<TriangularFace>::iterator fit = _face.begin(); fit != _face.end();
       ++fit)
    f >> *fit;
...
GLboolean
TriangulatedMesh3::SaveToOFF(const std::string& file_name) const
{ //----------------------------------------------
...
GLfloat*
TriangulatedMesh3::MapVertexBuffer(GLenum access_flag) const
{
...
GLfloat*
TriangulatedMesh3::MapNormalBuffer(GLenum access_flag) const
{ // homework
...
GLfloat*
TriangulatedMesh3::MapTextureBuffer(GLenum access_flag) const
{ // homework
...
GLvoid
TriangulatedMesh3::UnmapVertexBuffer() const
{
  glBindBuffer(GL_ARRAY_BUFFER, _vbo_vertices);
...
GLvoid
TriangulatedMesh3::UnmapNormalBuffer() const
{ // homework
...
GLvoid
TriangulatedMesh3::UnmapTextureBuffer() const
{ // homework
...
GLuint
TriangulatedMesh3::VertexCount() const
{ // homework
  return _vertex.size();
}
GLuint
TriangulatedMesh3::FaceCount() const
{ // homework
  return _face.size();
}
