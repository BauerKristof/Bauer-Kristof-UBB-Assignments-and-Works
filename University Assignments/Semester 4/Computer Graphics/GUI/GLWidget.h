#pragma once

#include <GL/glew.h>
#include <QGLWidget>
#include <QGLFormat>
#include <QTimer>
#include "../Parametric/ParametricCurves3.h"
#include "../Parametric/ParametricSurfaces3.h"
#include "../Core/TriangulatedMeshes3.h"
#include "../Core/Constants.h"
#include "../Core/Lights.h"


namespace cagd
{
    class GLWidget: public QGLWidget
    {
        Q_OBJECT

    private:

        // variables defining the projection matrix
        double       _aspect;            // aspect ratio of the rendering window
        double       _fovy;              // field of view in direction y
        double       _z_near, _z_far;    // distance of near and far clipping planes

        // variables defining the model-view matrix
        double       _eye[3], _center[3], _up[3];

        // variables needed by transformations
        int         _angle_x, _angle_y, _angle_z;
        double      _zoom;
        double      _trans_x, _trans_y, _trans_z;


        // your other declarations
        int hazi;

        //Lab2
        int _parametric_curve_index;
        ParametricCurve3* _pc;
        GenericCurve3* _image_of_pc;
        RowMatrix<ParametricCurve3*> _pcs;
        void render_parametric_curves();

        //Lab3
        QTimer *_timer;
        GLfloat _angle;
        TriangulatedMesh3 _mouse;
        int _parametric_surface_index;

        ParametricSurface3* _ps;
        TriangulatedMesh3*  _image_of_ps;
        RowMatrix<ParametricSurface3*> _pss;
        void render_parametric_surfaces();

        //Lab3 PlayGround
        Matrix<TriangulatedMesh3> _platform;


    private slots:
         void _animate();
    public:
        // special and default constructor
        // the format specifies the properties of the rendering window
        GLWidget(QWidget* parent = 0, const QGLFormat& format = QGL::Rgba | QGL::DepthBuffer | QGL::DoubleBuffer);

        // redeclared virtual functions
        void initializeGL();
        void paintGL();
        void resizeGL(int w, int h);

        virtual ~GLWidget();

        void create_parametric_curves();

        void create_parametric_surfaces();
        void create_playground();
        void render_playground();


    public slots:
        // public event handling methods/slots
        void set_angle_x(int value);
        void set_angle_y(int value);
        void set_angle_z(int value);

        void set_zoom_factor(double value);

        void set_trans_x(double value);
        void set_trans_y(double value);
        void set_trans_z(double value);

        void load_parametric_curve(int);

        void load_parametric_surfaces(int);

    };
}
