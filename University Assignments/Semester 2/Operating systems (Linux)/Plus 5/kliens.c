...
int main(int argc, char *argv[])          /* a számot a parancssorban adjuk meg */
{
  ...
  {
    printf("hasznalat:%s tipp (exit kilepeshez)\n",argv[0]);
    exit(1);
  }
	...
	if( read(fd1, &buffer, sizeof(buffer)) > 0)
	 {
      		printf("%s\n",buffer);
       		...
       		/* a válasz */
 	if(strcmp(buffer,"nyertel") ==0  ) {break;}
	if(strcmp(buffer,"vesztettel")==0) {printf("Mar kitalaltak \n");break;}
	else {printf("Adj meg egy masik szamot\n"); scanf("%s",t.parancs);}
